...
We start by looking at the posteriors for the spread (i.e., the SD) of the individual and the dyadic component.
...
\texttt{dur\_gamma0} (experimental) & positive continuous including 0 & positive continuous\\ 
\texttt{binary} (experimental) & 0 or 1 & positive continuous\\ 
\hline
\end{tabular}
\end{table}
...
```{r, echo=3:30, fig.width=8, out.width="95%", fig.height=2.4, fig.cap="\\label{fig:pp03}Posterior predictive checks for selected individuals."}
par(family = "serif", mgp = c(1.5, 0.5, 0), mar = c(2.5, 2.5, 0.5, 0.5))
par(mfrow = c(1, 3))
...
ridge_plot(fit1, greg = FALSE, sel_subset = c("F_@_J", "M_@_N", "D_@_L"),
           vert_exp = 10)
ridge_plot(fit1, greg = FALSE, sel_subset = c("F", "A_@_B", "D_@_L"),
           vert_exp = 10)
```
...
# A comparative example with empirical data

Here we illustrate the case of applying the model to multiple groups.
In this particular case, we have data on grooming interactions in groups of four macaque species, i.e. four groups in total.
These data could also come from different groups of the same species, or the same group recorded multiple times over time.
In this example here, we take the simplest case possible. 
We essentially treat the four groups as entirely independent entities.
In other words, if we were to fit the simple model from above (see figure \ref{fig:syl_net_soc}) four time, we would get very similar results.
Currently, this multi-group model only supports interaction data in the form of frequencies (`"count"`), but will eventually be extended to the remaining error families.

We start by creating a list of the input interaction data and a list of the corresponding observation effort.
For speed reasons we limit each group to 6 females for illustration.

```{r}
data(grooming)
# assemble list
g <- list(ass = grooming$ass$groom[1:6, 1:6],
          fus = grooming$fus$groom[1:6, 1:6],
          nig = grooming$nig$groom[1:6, 1:6],
          syl = grooming$syl$groom[1:6, 1:6])
o <- list(ass = grooming$ass$obseff[1:6, 1:6],
          fus = grooming$fus$obseff[1:6, 1:6],
          nig = grooming$nig$obseff[1:6, 1:6],
          syl = grooming$syl$obseff[1:6, 1:6])
standat <- bamoso:::make_stan_data_from_matrices_multi(
  bmats = g, 
  btypes = "count",
  omats = o
)
...
Now we proceed to fitting the model:

```{r macaque_multi_model, cache=TRUE, include=FALSE}
mac <- sociality_model(standat, parallel_chains = 4, 
                       refresh = 200,
                       adapt_delta = 0.97, seed = 47)
```

```{r, eval = FALSE}
mac <- sociality_model(standat, adapt_delta = 0.9)
```


We can plot the visual summaries as before, and while you could put all groups in one density the better choice is to visually separate them.

```{r, echo=5:15, fig.width=6, fig.height=4.5, out.width="69%", fig.align='center', fig.cap=figcap}
figcap <- "
Posterior distributions of individual gregariousness and dyadic affinity parameters.
The four underlying interaction networks were undirected grooming networks (frequencies or durations) in groups of 15 to 21 adult female macaques of different species.
"
par(mfcol = c(2, 2), family = "serif", mgp = c(1.5, 0.5, 0), mar = c(3, 3, 2, 1), tcl = 0)
maxy <- 3.2
sociality_plot(mac, add_prior = TRUE, ylim = c(0, maxy), group = "ass")
title(main = "assamensis", font.main = 4, line = -1)
sociality_plot(mac, add_prior = TRUE, ylim = c(0, maxy), group = "fus")
title(main = "fuscata", font.main = 4, line = -1)
sociality_plot(mac, add_prior = TRUE, ylim = c(0, maxy), group = "nig")
title(main = "nigra", font.main = 4, line = -1)
sociality_plot(mac, add_prior = TRUE, ylim = c(0, maxy), group = "syl")
title(main = "sylvanus", font.main = 4, line = -1)
```

We can also get numeric summaries.
...
We don't show the output here because it is quite extensive.
The posterior checks that are appropriate given the data and design constraints also work.
For example:

```{r, eval = FALSE}
pp_model(mac, group = "ass")
pp_model_stat(mac, stat = "max", group = "nig")
```
...
As mentioned repeatedly in this document, we make the assumption that if we model multiple behaviors that these can be explained by one axis.
Here we use an extreme example (hat tip to an anonymous reviewer who brought this example up!) to demonstrate what happens if this assumption is violated and what to look for if it is.
We start with generating one(!) behavior (proportions with an observation effort of 100 'trials') and extract the interaction matrix and the observation effort matrix.

```{r}
set.seed(123)
d <- generate_data(n_ids = 15, n_beh = 1, behav_types = "prop", prop_trials = 100,
                   indi_sd = 1, dyad_sd = 1, beh_intercepts = -1.5)
o1 <- d$processed$obseff_matrices[[1]]
m1 <- d$processed$interaction_matrices[[1]]
```

Now we 'reverse' the behavior, so if a dyad was observed 30 times out of 100, we now assign this dyad 70.
We do this for the entire interaction matrix.

```{r}
m2 <- o1 - m1
```

What we did here is essentially creating two behaviors that are perfectly negatively correlated.
In other words, with these two 'observed' behaviors we now know that our assumption is violated.
Let's fit our simple model and look how we can detect this.

```{r reviewerexample}
d <- make_stan_data_from_matrices(mats = list(b1 = m1, b2 = m2), 
                                  behav_types = c("prop", "prop"),
                                  obseff = list(b1 = o1, b2 = o1))

r <- sociality_model(d, parallel_chains = 4, silent = TRUE)
```


This model should sample without any computational issues.
The first hint that something went wrong can be seen in the summary:

```{r}
summary(r)
```

Both estimated SDs are practically 0, and without munch uncertainty at that!
This might make sense statistically, but biologically not so much.

More saliently, if we look at how the model performed in terms of predictions there is no doubt that the model we fitted is bad.

```{r, echo = 2:8, fig.width=8, fig.height=3, out.width="50%", fig.align='center', fig.cap="\\label{fig:ppextreme1}Posterior predictive checks for two behaviors."}
par(family = "serif", mgp = c(1.5, 0.5, 0), mar = c(2.5, 2.5, 0.5, 0.5), mfrow = c(1, 2))
pp_model(r, xvar = "b1", xlab = "behavior 1", ylab = "")
pp_model(r, xvar = "b2", xlab = "behavior 2", ylab = "")
```

Maybe you can already see what's going on?
For each behavior, the model predicts all dyads to interact equally often (roughly).
Here are few more checks to see:

```{r, echo = 2:8, fig.width=8, fig.height=3, out.width="50%", fig.align='center', fig.cap="\\label{fig:ppextreme2}Posterior predictive checks for two behaviors."}
par(family = "serif", mgp = c(1.5, 0.5, 0), mar = c(2.5, 2.5, 0.5, 0.5), mfrow = c(1, 2))
pp_model_stat(r, xvar = "b1", stat = "max", xlim = c(20, 100), 
              main = "", xlab = "behavior 1 (max)")
pp_model_stat(r, xvar = "b2", stat = "max", xlim = c(70, 100), 
              main = "", xlab = "behavior 2 (max)")
```

The model predicts maximum interaction proportions that are far away from the observed maximum value in our 'observed data' (and much more so in behavior 1 compared to behavior 2).

The take-home here is that we need to look at more than numeric summaries to see how our model performs.
In its own neat world, our model here works fine, except it is pretty much useless when we actually want to say anything about the behavior we have observed.
And that's what we sometimes forget (at least I have this impression very often).

So yes, our model here that assumes the existence of one gregariousness and one affinity axis will not always be appropriate.
But it might be useful (in its relative simplicity) to reflect reasonably well *observed* interaction patterns.

Sadly, the judgment regarding whether our model is good enough will require some subjective assessment in one way or other.
...
bamoso::pp_model_stat(r1, 1, "max", xlim = c(20, 70),
                      main = "max of behaviour 1")

bamoso::pp_model_stat(r2, 2, "iqr", xlim = c(20, 50), main = "")
bamoso::pp_model_stat(r1, 2, "iqr", xlim = c(20, 50),
                      main = "IQR of behaviour 2")

bamoso::pp_model_stat(r2, 3, "iqr", xlim = c(10, 30), main = "")
bamoso::pp_model_stat(r1, 3, "iqr", xlim = c(10, 30),
                      main = "IQR of behaviour 3")
...
abline(v = rep(gregmat[1, 2], 2), col = c("darkgrey", "#3B99B1B3"),
       lwd = c(3.5, 2))
abline(v = rep(affimat[1, 2], 2), col = c("darkgrey", "#EACB2BB3"),
       lwd = c(3.5, 2))

sociality_plot(r2, which_cor = c(1, 3), xlim = c(-1, 1))
abline(v = rep(gregmat[1, 3], 2), col = c("darkgrey", "#3B99B1B3"),
       lwd = c(3.5, 2))
abline(v = rep(affimat[1, 3], 2), col = c("darkgrey", "#EACB2BB3"),
       lwd = c(3.5, 2))

sociality_plot(r2, which_cor = c(2, 3), xlim = c(-1, 1))
abline(v = rep(gregmat[2, 3], 2), col = c("darkgrey", "#3B99B1B3"),
       lwd = c(3.5, 2))
abline(v = rep(affimat[2, 3], 2), col = c("darkgrey", "#EACB2BB3"),
       lwd = c(3.5, 2))
```
...
abline(v = rep(gregmat[1, 1], 2), col = c("darkgrey", "#3B99B1B3"),
       lwd = c(3.5, 2))
abline(v = rep(affimat[1, 1], 2), col = c("darkgrey", "#EACB2BB3"),
       lwd = c(3.5, 2))

sociality_plot(r2, which_beh = 2, xlim = c(0, 3))
abline(v = rep(gregmat[2, 2], 2), col = c("darkgrey", "#3B99B1B3"),
       lwd = c(3.5, 2))
abline(v = rep(affimat[2, 2], 2), col = c("darkgrey", "#EACB2BB3"),
       lwd = c(3.5, 2))

sociality_plot(r2, which_beh = 3, xlim = c(0, 3))
abline(v = rep(gregmat[3, 3], 2), col = c("darkgrey", "#3B99B1B3"),
       lwd = c(3.5, 2))
abline(v = rep(affimat[3, 3], 2), col = c("darkgrey", "#EACB2BB3"),
       lwd = c(3.5, 2))
```
...
One of the advantages or challenges of Bayesian modeling (depending on your perspective) is that we can or have to set priors.
In a nutshell (and simplified), priors are what we think the parameters that we want to estimate look like before we have modeled any data (or before the model has seen the data if you will).\footnote{Somewhat less informal: priors express what we believe about the probability distributions of our parameters before the data influence them through the likelihood.}
We don't offer a proper introduction to Bayesian modeling here, but just want to illustrate what consequences different priors can have and how to actually look at the consequences.

Practically speaking, in a lot of circumstances, priors do work well if left at some reasonable default values\footnote{Of course, what exactly 'reasonable default' means is highly context dependent, bummer.}.
And all packages that do Bayesian inference that I am aware of, use default priors in the sense that you can fit a model in say `brms`, `STRAND`, `bisonR` or `EloSteepness` and not even realize that there are priors involved.
It is reasonable to assume that the authors of these packages tried their best to set the default priors such they are sensible in a wide range of potential applications.
And this is what we tried here, too: the priors we chose should work in a wide range of data sets.
It's also important to remember that there are no right or wrong priors.
The only criterion that priors must meet is that they are justifiable in the light of our model (and while this is not an actual quote, I took it from somewhere but don't remember the source).

In the case of our sociality model here we estimate three key parameters: one intercept and two standard deviations.
We will simulate a data set in which we *know* the parameter values.
We then fit three models, the first with the default priors provided by `bamoso`.
And then we change the priors to fairly different from the defaults and observe the consequences on posterior distributions of our parameters.

Let's start with illustrating the exponential distribution, which is the distribution we use (see figure \ref{fig:priordistros}).
It's a common distribution to encode prior distributions of standard deviations.
The default setting used in the package (purple area in figure \ref{fig:priordistros}) puts half of the possible values between about 0.15 and 0.7. 
But it also allows larger values, although it becomes increasingly skeptical of values for an SD larger than 2 but importantly doesn't rule them out.
In contrast, the green (non-default) area is much more concentrated towards 0, i.e. much more skeptical of large values.
And that again contrasts with the orange prior, which appears almost flat\footnote{but it isn't actually flat!}, i.e., it allows for a much wider range of values for the SD.


```{r, echo=FALSE}
figcap <- "
Default and non-default prior distributions used currently in \\texttt{bamoso} for the gregariousness and affinity parameters.
\\label{fig:priordistros}
"
```


```{r, echo = FALSE, fig.width=3.8, fig.height=2.5, out.width="50%", fig.align='center', fig.cap = figcap}
par(family = "serif", mgp = c(1.5, 0.5, 0), las = 1, mar = c(3, 3, 1, 1), tcl = 0)
default <- rexp(10000, rate = 2)
flattish <- rexp(10000, rate = 0.5)
towards0 <- rexp(10000, rate = 5)

plot(density(towards0), type = "n", ylim = c(0, 5), yaxs = "i", xlim = c(0, 3),
     xaxs= "i", xlab = "x", ylab = "",
     main = "", axes = FALSE)
axis(1, at = 0:3, tcl = -0.2)
title(ylab = "density", line = 0.5)
box(bty = "l")
polygon(density(towards0, adjust = .4), col = adjustcolor("#009634", 0.7))
polygon(density(flattish, adjust = .4), col = adjustcolor("#CD7925", 0.7))
polygon(density(default, adjust = .4), col = adjustcolor("#8A1B60", 0.7))


text(0.15, 3, "rate = 5", adj = 0, col = adjustcolor("#009634", 1))
text(0.5, 1, "rate = 2 (default)", adj = 0, col = adjustcolor("#8A1B60", 1))
text(2, 0.5, "rate = 0.5", adj = 0, col = adjustcolor("#CD7925", 1))

```


The thing is that the posterior (which is what we want) is a trade-off between prior and data.
If the prior is nearly flat and there is a reasonable amount of information in the data then the prior doesn't matter much in determining the shape and location of the posterior.
If the prior is fairly informative (like the green one in figure \ref{fig:priordistros}) and there is no information in the data (the data are not "strong" enough) then the posterior might look very similar to the prior.
If the prior is fairly informative *and* the data are very informative then there can be a conflict.
For example, imagine the true value for the SD is 2, and the data strongly support that, but the prior is very skeptical about values larger than 1.
In such cases the prior 'pulls' the data towards itself, and the posterior ends up looking like a compromise between the two.


So what does that all mean with respect to our sociality model.
In practice, all the SDs that we have seen so far fall in the range between 0 and 2.
This is based primarily on simulations, in which priors like that lead to interaction data that looked *plausible*.
The few empirical data sets that we've seen appear to support that this range also by and large applies to the 'real world'.
Thus we decided for setting the purple area, i.e., a exponential distribution with rate parameter = 2.

Now, below we demonstrate with a simulated data set how we can change the priors and what the consequences are.
In short, the defaults work well (but that's hardly surprising because I simulated the data...).
If you look at figure \ref{fig:priorconsequences}a we see the posteriors with the default priors.
In panels b and c, we used combinations of non-default priors and you can see that even the much more informed priors pull the posteriors only to some extent towards themselves (affinity in panel b and gregariousness in panel c).

So, you can change priors in the call to `sociality_model`, but unless you have good reason to so I wouldn't recommend it.


```{r priorexample1datagen}
set.seed(1)
d <- generate_data(n_ids = 14, n_beh = 1,
                   behav_types = "count",
                   indi_sd = 1.1, dyad_sd = 1.1,
                   beh_intercepts = -1,
                   count_obseff = c(0.8, 1.8))

behmat <- d$processed$interaction_matrices[[1]]
colnames(behmat) <- rownames(behmat) <- LETTERS[1:d$standat$n_ids]
obsmat <- d$processed$obseff_matrices[[1]]
colnames(obsmat) <- rownames(obsmat) <- LETTERS[1:d$standat$n_ids]

sdat <- make_stan_data_from_matrices(mats = list(groom = behmat),
                                     behav_types = "count",
                                     obseff = list(obsmat))
...
```{r priorexample1mod1}
priors1 <- list(indi_sd = 2, dyad_sd = 2)
res1 <- sociality_model(sdat, parallel_chains = 4, silent = TRUE, 
                      refresh = 0, seed = 1, priors = priors1)
```

```{r priorexample1mod2}
priors2 <- list(indi_sd = 0.5, dyad_sd = 5)
res2 <- sociality_model(sdat, parallel_chains = 4, silent = TRUE, 
                      refresh = 0, seed = 1, priors = priors2)
```

```{r priorexample1mod3}
priors3 <- list(indi_sd = 5, dyad_sd = 0.5)
res3 <- sociality_model(sdat, parallel_chains = 4, silent = TRUE, 
                      refresh = 0, seed = 1, priors = priors3)
```
...
```{r, echo=FALSE}
figcap <- "
Consequences of setting different priors on the estimation of gregariousness and affinity.
In panel (a) both prior distributions have the same rate parameter of 2 and this is also the default setting.
In panel (b) the affinty prior has rate 5 and the gregariousness prior has rate 0.5.
In panel (c) the affinty prior has rate 0.5 and the gregariousness prior has rate 5.
\\label{fig:priorconsequences}
"
```

```{r, fig.width=6, fig.height=2, out.width="90%", fig.cap=figcap}
par(family = "serif", mgp = c(1.5, 0.5, 0), las = 1, mar = c(3, 3, 1, 1), tcl = 0)
par(mfrow = c(1, 3))

ymax <- 4

sociality_plot(res1, ylim = c(0, ymax), xlim = c(0, 3))
set.seed <- 1
ipriors1 <- rexp(1000, priors1$indi_sd)
dpriors1 <- rexp(1000, priors1$dyad_sd)
points(density(ipriors1), type = "l", col = "#3B99B1", lwd = 4)
points(density(dpriors1), type = "l", col = "#EACB2B", lwd = 4)
points(density(ipriors1), type = "l", lwd = 0.5)
points(density(dpriors1), type = "l", lwd = 0.5)
text(0.1, ymax, "(a)", xpd = TRUE, adj = c(0, 1))

sociality_plot(res2, ylim = c(0, ymax), xlim = c(0, 3))
set.seed <- 1
ipriors2 <- rexp(1000, priors2$indi_sd)
dpriors2 <- rexp(1000, priors2$dyad_sd)
points(density(ipriors2), type = "l", col = "#3B99B1", lwd = 4)
points(density(dpriors2), type = "l", col = "#EACB2B", lwd = 4)
points(density(ipriors2), type = "l", lwd = 0.5)
points(density(dpriors2), type = "l", lwd = 0.5)
text(0.1, ymax, "(b)", xpd = TRUE, adj = c(0, 1))

sociality_plot(res3, ylim = c(0, ymax), xlim = c(0, 3))
set.seed <- 1
ipriors3 <- rexp(1000, priors3$indi_sd)
dpriors3 <- rexp(1000, priors3$dyad_sd)
points(density(ipriors3), type = "l", col = "#3B99B1", lwd = 4)
points(density(dpriors3), type = "l", col = "#EACB2B", lwd = 4)
points(density(ipriors3), type = "l", lwd = 0.5)
points(density(dpriors3), type = "l", lwd = 0.5)
text(0.1, ymax, "(c)", xpd = TRUE, adj = c(0, 1))
```
...
The simulations in this section should demonstrate what happens if we use only the priors to estimate the parameters of interest.
Actually, we still need the data because we require the observation effort (if present) and the interactions (to determine the priors for the behavioral intercept(s)).

```{r, eval = FALSE}
set.seed(123)
x <- generate_data(n_ids = 34, n_beh = 1, behav_types = "count",
...
The posterior means for both variance components should be around 0.5 (which is the median of the prior distribution (exponential(rate = 2)))\footnote{or the posterior \textit{medians} would be around 0.33}.
...
pp_model_dens(r, ylim = c(0, 0.5), xlim = c(0, 50), n_draws = 100, xadjust = 2)
```
...
In order to bring the individual level parameters (gregariousness SD and individual gregariousness values) to the same scale, we need to multiply by $\sqrt{0.5}$.\footnote{There is also the possibility to make the adjust in the call to \texttt{brm()}, but I find this too convoluted:\\\\
\texttt{dyads\$w <- cbind(rep(sqrt(0.5), nrow(dyads)),  rep(sqrt(0.5), nrow(dyads)))}\\\\
\texttt{brm(... + (1|mm(id1, id2, weights = w, scale = FALSE)), ...)}
}
<!-- The code below demonstrates this without showing actual results. -->
The resulting `brms` estimates correspond fairly closely to the ones from `bamoso` (see figures \ref{fig:brmscomparefig1} and \ref{fig:brmscomparefig2}).

```{r, eval = do_brms_compare}
set.seed(50)
...
x <- generate_data(n_ids = 20,
                   n_beh = 1, 
                   beh_intercepts = true_intercept,
                   behav_types = c(approach = "count"), 
                   count_obseff = 10,
                   indi_sd = true_indi_sd, 
                   dyad_sd = true_dyad_sd)
...
```{r brmscomparecomputations, cache = TRUE, eval = do_brms_compare, include = FALSE}
library(brms)
bres <- brm(approach ~ offset(log(obseff)) + (1|dyad) + (1|mm(id1, id2)),
            data = dyads, family = poisson, refresh = 0,
            backend = "cmdstanr", seed = 2, cores = 4)
bamres <- sociality_model(standat, parallel_chains = 4, silent = TRUE,
                          seed = 123, show_exceptions = FALSE)
```


```{r, echo=FALSE}
cap <- "
Fitting the simplest version of the sociality model is also possible using the multi-membership syntax in \\texttt{brms}.
The resulting posteriors of the key parameters are very similar to those obtained from \\texttt{bamoso} package.
"
```


```{r brmscomparefig1, echo = 2:50, eval = do_brms_compare, fig.width=6, fig.height=2.3, out.width="70%", fig.align='center', fig.cap=cap}
par(mgp = c(1.6, 0.3, 0), mar = c(2.5, 2.5, 1, 1), tcl = -0.1, las = 1, family = "serif")

plot(0, 0, xlim = c(-1, 2), xaxs = "i", ylim = c(0, 6), yaxs = "i", type = "n", 
     xlab = "estimate", ylab = "density", bty = "l")
cols <- adjustcolor(hcl.colors(3, palette = "zissou"), 0.5)

legend(1.5, 6, xpd = TRUE, bty = "n", pt.cex = 1.5, cex = 0.7,
       legend = c("true", "bamoso", "brms", "individual", "dyadic", "baseline"), 
       lty = c(1, 3, 2, NA, NA, NA), 
       pch = c(NA, NA, NA, 15, 15, 15),
       col = c("black", "black", "black", cols[1], cols[2], cols[3])
)
...
abline(v = c(true_indi_sd, median(res_brms), median(res_bam)), 
       lty = c(1, 2, 3), col = cols[1], lwd = 2)
...
abline(v = c(true_dyad_sd, median(res_brms), median(res_bam)), 
       lty = c(1, 2, 3), col = cols[2], lwd = 2)
...
abline(v = c(true_intercept, median(res_brms), median(res_bam)), 
       lty = c(1, 2, 3), col = cols[3], lwd = 2)
...
```{r, echo=FALSE}
cap <- "
The estimated individual level gregariousness values and the dyadic affinity values also correspond very well between the \\texttt{brms} and \\texttt{bamoso} implementations.
"
```

```{r brmscomparefig2, echo = 2:50, eval = do_brms_compare, fig.width=8, fig.height=2.2, out.width="95%", fig.align='center', fig.cap=cap}
par(mfrow = c(1, 2), mgp = c(1.6, 0.3, 0), tcl = -0.1, mar = c(3, 3, 1, 1), las = 1, family = "serif")
